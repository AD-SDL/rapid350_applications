...
class AMESExperimentApplication(
    ...
    def define_starting_resources(
        ...
        deep_well_48_resource = Collection(
            resource_name=f"AMES_deep_well_48_set{experiment_set}",
            resource_description=f"48-well deep well plate resource set {experiment_set}",
            capacity=2,
            children={
                    # "lid_slot": Slot(
        ...
        for i in range(3):
            ...
            current_lid_resource = Resource(
                resource_name = f"AMES_microplate_lid_set{experiment_set}_plate{i+1}",
            ...
            current_microplate_resource = Collection(
                resource_name = f"AMES_microplate_384well_set{experiment_set}_plate{i+1}",
    ...
    def run_app(
        ...
        #for i in range(2):  # 2 loops total here!!
        for i in range(1):  # FOR TESTING!
            ...
            # TODO: TEST WITH PLATE ID INPUT INTO LICONIC!!!
            ...
            # TODO: TEST!!!
            # TODO: Record the time that each exp/indic plate starts incubating
            exposure_indicator_incubation_start_times.append(int(time.time()))

            # TESTING
            print(f"STARTED INCUbATION AT {exposure_indicator_incubation_start_times[i]}")

            # TODO: Change this to user hits enter once the tips are replaced.
            print("PLEASE REPLACE THE TIPS AT POS 5 in the next 60 seconds!")
            time.sleep(60)


        # Wait for the first exp/ind plate to finish incubating
        # 7. Incubate at 37C for 90 min, with gentle shaking.
        # first_incubation_start_time = time.time()
        print(f"Incubating for {exposure_incubation_time} seconds.")
        while time.time() - exposure_indicator_incubation_start_times[0] < exposure_incubation_time:
            print(f"Incubating. Continuing in {int(exposure_incubation_time - (time.time() - exposure_indicator_incubation_start_times[0]))} seconds.")
            time.sleep(5)

            if time.time() - exposure_indicator_incubation_start_times[0] >= exposure_incubation_time:
        ...
        # # 9. Refill the tips before the exposure to indicator transfer
        # self.workcell_client.submit_workflow(
        #     workflow_definition = refill_tips_wf,
        #     json_inputs={
        #         "tip_box_position": parameters["tip_box_position"],
        ...
        # # 10. Run SOLO protocol: Transfer FIRST exposure column to FIRST indicator column and MIX WELL
        # hso_5, hso_5_lines, hso_5_basename = package_hso(
        #     exposure_to_indicator_1.generate_hso_file, parameters, "/home/rpl/workspace/madsci_temp/solo_temp5.hso"
        # )
        ...
        # # 11. Run SOLO protocol: Transfer SECOND exposure column to SECOND indicator column and MIX WELL
        # hso_6, hso_6_lines, hso_6_basename = package_hso(
        #     exposure_to_indicator_2.generate_hso_file, parameters, "/home/rpl/workspace/madsci_temp/solo_temp6.hso"
        # )
        # parameters["protocol_file"] = "/home/rpl/workspace/madsci_temp/solo_temp6.hso"
        # self.workcell_client.submit_workflow(
        #     workflow_definition = run_solo_wf,
        #     file_inputs={
        #         "protocol_file": parameters["protocol_file"],
        ...
        # # 12. Run SOLO protocol: Transfer THIRD exposure column to THIRD indicator column and MIX WELL
        # hso_7, hso_7_lines, hso_7_basename = package_hso(
        #     exposure_to_indicator_3.generate_hso_file, parameters, "/home/rpl/workspace/madsci_temp/solo_temp7.hso"
        # )
        # parameters["protocol_file"] = "/home/rpl/workspace/madsci_temp/solo_temp7.hso"
        # self.workcell_client.submit_workflow(
        #     workflow_definition = run_solo_wf,
        #     file_inputs={
        #         "protocol_file": parameters["protocol_file"],
        ...
        #     parameters["half"] = 1
        #     solo_temp_filename = f"/home/rpl/workspace/madsci_temp/solo_temp_384_{i+7}.hso"
        ...
        #     parameters["half"] = 2
        #     solo_temp_filename = f"/home/rpl/workspace/madsci_temp/solo_temp_384_{i+8}.hso"
        ...
        # # 16. Incubate all 3 384-well plates overnight (48 hours, no shaking, 37C).
        # microplate_incubation_start_time = time.time()
        # print(f"Incubating for {microplate_incubation_time} seconds.")
        # while time.time() - microplate_incubation_start_time < microplate_incubation_time:
        #     print(f"Incubating. Continuing in {int(microplate_incubation_time - (time.time() - microplate_incubation_start_time))} seconds.")
        #     time.sleep(5)

        #     if time.time() - microplate_incubation_start_time >= microplate_incubation_time:
        #         print("Incubation complete")
        #         break
